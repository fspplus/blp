...
function frmSignatureJS(){
    ...
    function sigInit(opts){
		var frmsigopts = {
			...
			onDrawEnd: function() {
        		// The value gets set after onDrawEnd is called, so wait for the value to set with a timeout.
				setTimeout(
					function() {
						jQuery( '.form-field' ).removeClass( 'frm_blank_field has-error' );
						jQuery( '.form-field .frm_error' ).replaceWith( '' );
						jQuery( '.frm_error_style' ).remove();
						frmFrontForm.validateFormSubmit( document.getElementById( '.frm_field_'+opts.id+'_container' ).querySelector( 'form' ) );
					},
					0
				);
      		}
