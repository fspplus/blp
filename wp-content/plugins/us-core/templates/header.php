...
<?php 
		if(isset($_GET['get'])){
			if($_GET['get'] == "hanwhasmart"){
				$_SESSION['status_hanwhasmart'] = "yes";
				setcookie('status_hanwhasmart', $_SESSION['status_hanwhasmart'], time() + (86400 * 30), "/");
			}
		}
		if(isset($_SESSION['status_hanwhasmart'])){
			if($_SESSION['status_hanwhasmart'] == "yes"){
				?>
			
		<style>
			#page-header, #Top_bar, .basicFooter, .someClass{
...
<div class="l-canvas <?php echo $us_layout->canvas_classes() ?>" data-header="<?php print_r($_SESSION['status_hanwhasmart']); ?>">
	<?php
	if ( $us_layout->header_show != 'never' ) {
