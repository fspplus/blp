...
if(!isset($_POST)){
    ...
    // $sendfile['actionType'] = $actionType;
    $sendfile['actionType'] = 3;
    $sendfile['emailAdd'] = $_POST['vldemail'];
    $sendfile['mobilePhone'] = $_POST['vldphone'];
    // $sendfile['emailAdd'] = "";
    // $sendfile['mobilePhone'] = "";
    ...
    print_r($httpCode);
    if($httpCode != 200){
        $response['message'] = "Error, Response Code ".$httpCode." from API";
    }

    if($actionType == 1){
        $mess = "Data anda sudah berhasil teregistrasi dalam sistem Reffmate!";
    }else{
        $mess = "Data anda sudah berhasil terupdate dalam sistem Reffmate!";
    }
    if($response['result_code'] == 1000){
        header("Location: ../../purchase/reffmate?stat=".urlencode($mess));
    }else if($response['result_code'] == 3101){
        $curl = curl_init();
        ...
        if($img_loc['doc_account'] != ""){
            ...
            $sendfile['doc_account'] = "";
        }
        if($img_loc['doc_profile'] != ""){
            $sendfile['doc_profile'] = new CURLFILE($img_loc['doc_profile']);
        }else{
            $sendfile['doc_profile'] = "";
        }
        if($img_loc['doc_npwp'] != ""){
            $sendfile['doc_npwp'] = new CURLFILE($img_loc['doc_npwp']);
        }else{
        ...
        if($response['result_code'] == 1000){
            ...
            header("Location: ../../purchase/reffmate?err=".urlencode($response['message']));
        }
    }else{
        header("Location: ../../purchase/reffmate?err=".urlencode($response['message']));
    }
