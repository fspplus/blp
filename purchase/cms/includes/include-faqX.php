<?php
    ...
    $pre_tab = mysqli_query($conn, "SELECT * FROM `faq_table`");
    $tab_active = mysqli_query($conn, "SELECT * FROM `faq_table` WHERE faq_status LIKE 'active'");
    $tab_inactive = mysqli_query($conn, "SELECT * FROM `faq_table` WHERE faq_status LIKE 'inactive'");
                ...
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-7 text-right pagedesktop">
                            	<span>Page: </span> 
                                <?php
                                	$currpage = $_GET['page'];
                                	if ($currpage == "") {
                            ...
                            <form class="table-form" action="./controller/batchprocess.php" method="post">
                        <div class="row">
                            <div class="col-lg-12 col-m-12 displayNone">
                                <div class="batchEdit"><span style="margin-right: 15px;">Batch Action</span>
                                    <select class="txedit" name="selectbatchaction" style="margin-right: 15px;" required>
                                        <option value="" disabled selected>--SELECT OPTION--</option>
                                        <option value="delete">Delete Selected</option>
                                        <option value="activate">Activate Selected</option>
                                        <option value="deactivate">Deactivate Selected</option>
                                    </select>
                                    <input type="submit" class="btnAddNew" value="Apply">
                                    <strong> *DO NOT USE! STILL UNDER DEVELOPMENT</strong>
                                </div>
                            </div>
                            <div class="col-lg-12 col-m-12">
                                <div class="">
                                    <label>
                                        <input type='checkbox' id='selectall' class='selectall' data-type='faq'>
                                ...
                                <div class="table-responsive table--no-card m-b-30">
                                    <table class="table table-borderless table-striped table-earning">
                                        ...
                                        <tbody>
                                            <?php
                                            	if ($status == "all" || $status == "") {
                                            		$sql = "SELECT * FROM `faq_table` ORDER BY  `faq_table`.`faq_id` DESC LIMIT ".$startpage.", ".$limitpage;
                                            		?><script>
                                            			$(function(){
                                            				$('#all').addClass("active");
                                            			});
                                            		...
                                            		$sql = "SELECT * FROM `faq_table` WHERE faq_status LIKE 'active'  ORDER BY  `faq_table`.`faq_id` DESC LIMIT ".$startpage.", ".$limitpage;
                                            		?><script>
                                            			$(function(){
                                            				$('#act').addClass("active");
                                            			});
                                            		...
                                            		$sql = "SELECT * FROM `faq_table` WHERE faq_status LIKE 'inactive' ORDER BY  `faq_table`.`faq_id` DESC LIMIT $startpage, $limitpage";
                                            		?><script>
                                            			$(function(){
                                            				$('#ict').addClass("active");
                                            			});
                                            ...
                                            ?>
                                                <tr>
                                                    <td ><input type="checkbox" name="selectObject[]" class="selectObject" value="<?php echo $data['faq_id']; ?>"><?php echo $data['faq_category']; ?></td>
                            ...
                            <div class="text-right pagemobile">
                                <span>Page: </span> 
                                <?php
                                    $currpage = $_GET['page'];
                                    if ($currpage == "") {
                                    ...
                                    if ($currpage != 1) {
                                        ?><a id="mpage<?php echo $prev_page;?>" class="btnpage" href="?status=<?php echo $status; ?>&page=<?php echo $prev_page; ?>"><?php echo $prev_page; ?></a><?php
                                    }
                                    ?>
                                    <a id="mpage<?php echo $currpage;?>" class="btnpage" href="?status=<?php echo $status; ?>&page=<?php echo $currpage; ?>"><?php echo $currpage; ?></a>
                                    <?php
                                    if ($currpage != $pages) {
                                        ?><a id="mpage<?php echo $next_page;?>" class="btnpage" href="?status=<?php echo $status; ?>&page=<?php echo $next_page; ?>"><?php echo $next_page; ?></a><?php
                                    }
                                    ...
                                    if ($currpage != "") {
                                        ?><script>
                                            $(function(){
                                                $('#mpage<?php echo $currpage;?>').addClass("active");
                                            });
                                        </script><?php
                                    }else{
                                        ?><script>
                                            $(function(){
                                                $('#mpage1').addClass("active");
                                            });
                                        </script><?php
                                    }
