    ...
    function verifyhash($hash){
        ...
        curl_setopt_array($curl, array(
          ...
          CURLOPT_HTTPHEADER => array(
            ...
            "domainserver:http://koreaversikamu.co.id"
          ),
        ));
        ...
        // echo "This is some things";
        // echo "urlnya: ".$response;
        return $data;
    }
...
function get_wparticles(){
    $base_url='https://hanwhalife.co.id';
    $total_page=0;
    $per_page=6;

    $ch = curl_init();
    $headers = [];
    curl_setopt($ch, CURLOPT_URL, "$base_url/wp-json/wp/v2/article_bucketlistpl?per_page=$per_page");
    ...
    $total_page=$headers['x-wp-totalpages'][0];

    $url="$base_url/wp-json/wp/v2/article_bucketlistpl?per_page=$per_page";
    if(isset($_GET['page'])){
        $url="$base_url/wp-json/wp/v2/article_bucketlistpl?per_page=$per_page&page=".$_GET['page'];
    }
    ...
    if ($err) {
        ...
        $data = json_decode($response, true);
        //echo "<h1 style='text-align: center;'>Artikel</h1> <div class='row' style='margin: 20px 0px;'>";
        foreach($data as $result){?>
            <div class="row">
                <div class="col-sm-4"><a href="http://uat.koreaversikamu.co.id/articles/<?php echo $result['slug']; ?>.html" class="">
                    <?=isset($result['better_featured_image']['media_details']['sizes']['medium']['source_url'])?
                    '<img src="'.$result['better_featured_image']['media_details']['sizes']['medium']['source_url'].'" class="img-responsive">':
                    '<img src="http://placehold.it/1280X720" class="img-responsive">'?>
                    </a>
                </div>
                <div class="col-sm-8">
                  <h2 class="title"><a class="art-title" href="http://uat.koreaversikamu.co.id/articles/<?=$result['slug']?>.html"><?=$result['title']['rendered']?></a></h2>
                  <!--<p class="text-muted"><span class="glyphicon glyphicon-lock"></span> Available Exclusively for Premium Members</p>-->
                  <p><?=substr(strip_tags($result['content']['rendered']),0,200)?>...</p>
            ...
            <!--<div class="col-4">
                <div class="col-12">
                    <a href="http://articles/<?php echo $result['slug']; ?>.html">
                        <h2 style="color: #ff7101; text-align: center;"><?php echo $result['title']['rendered']; ?></h2>
                    </a>
                </div>
                <div class="col-12">
                    <?php echo substr($result['content']['rendered'],0,200); ?>
                </div>
                <div class="col-12" style="text-align:center;margin-bottom:10px;">
                    <a href="http://articles/<?php echo $result['slug']; ?>.html" class="btn btn-small">Read More</a>
                </div>
            </div>-->
        <?php }
        echo '<div class="row"><div class="col-12">';
        echo '<div class="cstpagination" style="text-align:center;padding:0.5em;margin-top:32px;">';
            for($x=1;$x<=$total_page;$x++){
                echo '<p><a class="page" href="http://uat.hanwhalife.co.id/articles?per_page='.$per_page.'&page='.$x.'">'.$x.'</a></p>';
            }
        echo '</div></div></div>';
    }
}
function get_wparticles_detail(){
    //echo "<div style='padding-top: 25%;'></div>";
    //echo $_GET['title'];
    $base_url='https://hanwhalife.co.id';
    if(isset($_GET['title'])){
        $url="$base_url/wp-json/wp/v2/article_bucketlistpl?slug=".$_GET['title'];
    }else{
        return;
    }
    ...
    if ($err) {
        ...
        $data = json_decode($response, true);
        $data=$data[0];
        echo isset($data['better_featured_image']['media_details']['sizes']['medium_large']['source_url'])
            &&$data['better_featured_image']['media_details']['sizes']['medium_large']['source_url']!='null'
            &&$data['better_featured_image']['media_details']['sizes']['medium_large']['source_url']!=''?
                    '<img src="'.$data['better_featured_image']['media_details']['sizes']['medium_large']['source_url'].'" class="img-responsive">':
                    '';
        echo '<h3 style="text-align: left;margin-top:25px;">'.$data['title']['rendered'].'</h3>';
        echo '<div class="article-details">'.$data['content']['rendered'].'</div>';
        //echo '<div class="sharethis-inline-share-buttons"></div>';
        echo '<div class="sharethis-inline-share-buttons"></div><br/>';
        echo '<br/>';
    }
    /*$author_url="https://hanwhalife.co.id/wp-json/wp/v2/users/".$data['author'];
    $curlauthor = curl_init();

    curl_setopt_array($curlauthor, array(
    ...
    $responseauthor = curl_exec($curlauthor);
    $errauthor = curl_error($curlauthor);
    curl_close($curlauthor);
    $data_author = json_decode($responseauthor, true);
    echo '<p>by <i>'.$data_author['name'].'</i></p>';
    echo '<br/>';*/
}
function get_wparticles_categories($id=0){
    $curl = curl_init();

    curl_setopt_array($curl, array(
      CURLOPT_URL => "https://hanwhalife.co.id/wp-json/wp/v2/article_bucketlistpl?categories=$id&per_page=5",
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 30,
...
function referralLists($reffcode){
  $curl = curl_init();
...
function bankADlist(){
  ...
  curl_setopt_array($curl, array(
    CURLOPT_PORT => "8080",
    CURLOPT_URL => "http://192.168.70.70:8080/hanwhaservices/properties/bankad",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => "",
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
...
function date_converter($date){
  ...
  foreach($bulan as $stepper_bulan){
    if (strpos($date, $stepper_bulan) !== false) {
      $formatted_date = str_replace($stepper_bulan, $bulan_2[$stepper], $date);
...
function date_toID($date){
  $bulan_2 = array(1=>'Januari',
  'Februari',
  'Maret',
  'April',
  'Mei',
...
function reffmateIncome($reffID, $memberID="", $startDate="", $endDate=""){

  $key = keyapi();
  $curl = curl_init();

  curl_setopt_array($curl, array(
    CURLOPT_URL => 'http://192.168.70.70:8080/hanwhaservices/member/referralMate/collection',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 0,
    ...
    CURLOPT_CUSTOMREQUEST => 'POST',
    CURLOPT_POSTFIELDS =>'{
    "hanwhaReferenceCode": "'.$reffID.'",
    "periodeStartDate": "'.$startDate.'",
    "periodeEndDate": "'.$endDate.'",
    "memberId": "'.$memberID.'"
  }',
    CURLOPT_HTTPHEADER => array(
      $key,
